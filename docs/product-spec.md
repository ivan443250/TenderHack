...
4. Decisions: `ANSWER`, `CLARIFY`, `HANDOFF_OFFER`, `ANSWER_AND_HANDOFF`, `OUT_OF_SCOPE`, `MODERATION_WARNING`, `MODERATION_CLOSE`, `TECHNICAL_ERROR`.
...
| `OUT_OF_SCOPE` | Запрос явно относится к small talk или чужой теме: показывается scope guidance без knowledge answer и handoff |
