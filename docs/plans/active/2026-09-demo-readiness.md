...
- 2026-09-13: добавлена тестовая админ-панель `GET /admin` + `GET /admin/api/summary?from&to&limit` (`src/support-core/src/TenderHack.Api/Admin/`): read-only сводка по `cases/turns/case_events/feedback` за период, embedded HTML без связи с SPA, gate `Admin:Enabled`/`Admin:Token` (`ADMIN_PANEL_*` в compose/.env.example), 11 тестов в `Api.Tests`. Вне `web-api-v0`; ролей нет — на общем хосте обязателен токен. Может служить источником данных для будущего `/analytics` (product-enhancements F4), но сам им не является: считает только факты `api`, без quality-оценок knowledge.
